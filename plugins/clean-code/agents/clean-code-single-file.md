...
- **Principle selection**: "Apply ONLY these principles: [list]" or "Skip these principles: [list]". Skip steps for non-selected/excluded principles. Always execute Step #1 (read), Step #13 (verify), and Step #14 (report) regardless.
...
Upon receiving a file, execute these 14 steps in order. Track progress internally — do NOT create Tasks via any task-management tool (the user's Task view is reserved for the orchestrator):

### Step #1: Read file and prepare
...
### Step #6: Abstraction Level Consistency
...
- **Do NOT re-split** functions that were already extracted in Step #3. This principle reviews the remaining code, not the results of prior extraction.
...
### Step #9: Dead Code & Noise Removal
...
### Step #12: Magic Numbers/Strings → Named Constants
...
### Step #13: Verify public interface unchanged + format

1. Re-read the file and compare the current public interface (exported functions, types, class APIs) against what you noted in Step #1
